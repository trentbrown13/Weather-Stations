...
/**************************** MANUFACTING Calibration DEFINES *******************************************
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (strcmp(topic, TOPIC) == 0 ) {
    ...
    if ((char)message[0] == '1') {
      ...
      //publishUpgSwitchState(); // Current issue with matching on update not due to this if I comment it out
      checkForUpdates();
      //
       publishUpgSwitchState(); // Current issue with matching on update not due to this if I comment it outpublishUpgSwitchState(); // Current issue with matching on update not due to this if I comment it out
  }
  else {
...
void setup() {
 ...
 // digitalWrite(BUILTIN_LED, LOW); // Just added this 12/31
  ...
  */
  ...
  if (awake == true)
  {
    ...
    /*
      ...
      client.loop();
      delay(100);
      client.subscribe("wemosTestClient/setAdcAdjust");
      client.loop();
      delay(100);
